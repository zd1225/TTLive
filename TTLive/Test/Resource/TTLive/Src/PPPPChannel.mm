...
void CPPPPChannel::ProcessCommand(int cmd, char *pbuf, int len)
{
    NSLog(@" ppppchannel ProcessCommand uid %s cmd : 0x%x",szDID,cmd);
...
RE_CONNECT:
    ...
    {        
        ...
        NSString *uidUp = [uid uppercaseString];
        if ([uidUp rangeOfString:@"VSTA"].location != NSNotFound) {
            ...
            NSLog(@"PPPP_Connect %s return: %d", szDID, m_hSessionHandle);
        } else {
            m_hSessionHandle = PPPP_Connect(szDID, 1, 0);
        }
